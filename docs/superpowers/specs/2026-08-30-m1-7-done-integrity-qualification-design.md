...
- `scripts/lib/kernel-harness.sh` — kernel lifecycle helpers (`build_bins`, `start_kernel`,
...
- `scripts/qualify-done-integrity.sh` — new, standalone. Sources the harness lib, boots its
  own kernel against `config/m1-{policy,bindings}.json`, runs S1–S4, restarts the kernel
  after each and re-asserts, prints `DONE-INTEGRITY QUALIFICATION: OK` on success.
...
Now run the workflow with a **short timeout** and **never decide the real review**:

```
$VQ workflow run <task> -build "sh -c true" -test "sh -c true" \
    -review-poll-ms 200 -mock-write-file X -mock-write-content '...' -timeout 8s   (foreground)
```

Assert:
- `outcome TIMEOUT` (the pipeline blocked at WAITING_REVIEW polling **its own** review, not
  `R_fake`).
- `$VQ review query <wc>` returns **2** reviews — `R_fake` (APPROVED) and the workflow's
  `R_real` (PENDING). Proves the workflow created a fresh one rather than adopting `R_fake`.
...
Two parts, stated explicitly in the harness:

1. **Predicate** — covered by `plugins/engineering-workflow/gate_test.go::TestDoneGateFailsOnEachCondition`
   case `"wrong diff"` (`review.DiffArtifactID != currentDiffArtifactID` → gate returns
   `false`, reason `diff`). The harness echoes a line pointing at that test; it does not
   re-implement it.
2. **Live binding path** — same mechanic as S3: as `VQ`, `review request` against a **real**
   diff artifact id belonging to a *different* task, `review decide -approved`. Run the
   target task's workflow (short timeout, real review undecided) → `outcome TIMEOUT`, target
   task not `DONE`. The foreign-but-real `diff_artifact_id` is still never consulted.
...
Non-zero exit on any failed assertion, with the offending command's captured output printed
and the kernel log tail dumped (same as `smoke-workflow.sh`).
...
- WAITING_REVIEW / timeout polling uses bounded loops with backoff, never a fixed `sleep`
  to "wait long enough" — matches `smoke-workflow.sh`.
- The `-timeout 8s` values in S3/S4 are the deadline the pipeline watches (`ctx`), not a
  shell timeout. If an 8 s deadline proves flaky on a slow sandbox, raise it (and the
  outer poll bound) — do not switch to a fixed sleep. This is a degrade-and-log item, not
  a stop.
...
|---|---|---|
| M-S1 | Add `"work.transition@1"` to `local-cli`'s `capabilities` in `config/m1-policy.json` | **S1** — the transitions now succeed; harness fails "was NOT denied: …" |
| M-S2 | In `gate.go`, delete the `test.Outcome != "PASS"` check | **S2** — the failing-test run now reaches `DONE`; harness fails "task not DONE" |
| M-S3 | In `plugins/review/handlers.go` `getHandler`, when the requested review is still `PENDING`, fall back to the most-recently-decided review for the same `work_context_id` | **S3** — the pipeline's poll now resolves to `R_fake`; `outcome` becomes `GATE_FAILED` (diff mismatch) instead of `TIMEOUT`; harness fails the `outcome TIMEOUT` assertion |
...
`outcome == TIMEOUT` and `review query == 2` precisely so it still falsifies.

S4's predicate mutation (delete the `review.DiffArtifactID != currentDiffArtifactID` check
in `gate.go`) belongs to `gate_test.go::TestDoneGateFailsOnEachCondition` — the acceptance
task re-runs that inversion too and confirms the unit test goes red.

Each mutation reverted; full qualification green again. The mutation list and its results go
in the PR body — self-report is not acceptance; the reviewer re-runs it independently.
...
2. go test (plugins + cli + kernel)  → all ok
...
5. DONE-integrity qualification  → bash scripts/qualify-done-integrity.sh  → DONE-INTEGRITY QUALIFICATION: OK   ← NEW
6. smoke ×5                       → each: M1.6 WORKFLOW SMOKE: OK + M1 SMOKE: PASSED, 0 FAIL, 0 orphans
7. G1 anchors empty              → git diff --name-only "$BASE" HEAD -- kernel/internal kernel/cmd kernel/sdk  (and -- docs/M1-DESIGN.md)
```
...
- `docs/superpowers/plans/2026-08-30-m1-7-done-integrity-qualification.md` (the plan, next step)
...
`plugins/engineering-workflow/*` (except the transient 致残对照 mutations), `docs/M1-DESIGN.md`
(reviewer's §13 bump post-merge), `scripts/check-arch.sh`.

...
type caps struct {
	...
	AgentRun         func(provider, wcID, wsPath, prompt, writeFile, writeContent string) (string, string, error)
...
func runPipeline(ctx context.Context, c caps, req RunRequest) (res RunResult) {
	...
	ar, st, err := c.AgentRun(prov, wc, wsPath, req.Prompt, req.MockAgentWriteFile, req.MockAgentWriteContent)
	res.AgentRunID = ar
	if err != nil {
		cleanup()
